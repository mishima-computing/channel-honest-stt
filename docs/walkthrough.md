...
> [!IMPORTANT]
> **Held-out（話者オープン）での厳密な評価**
> MCTの学習に使用したデータ（jvs001〜jvs010）と、評価・キャリブレーション測定に使用したデータ（jvs011〜jvs020）は、**完全に話者レベルで分離（Held-out）されています**。同じデータで評価したことによる「過学習（自己欺瞞）」の可能性は完全に排除されており、提示しているキャリブレーションカーブはすべて真の汎化性能を示しています。
...
### MCTによる確信度の健全化（0dB）
最も過酷な0dB環境でも、MCTは絶大な効果を発揮しました。0dBのキャリブレーションカーブも以下に示します。

![MCT 0dB Calibration](assets/MCT_0dB_calibration.png)
![MCT 0dB Tradeoff](assets/MCT_0dB_tradeoff.png)
![MCT 0dB FP Dist](assets/MCT_0dB_uv_fricative_fp.png)

> [!WARNING]
> **0dBでのキャリブレーション（過酷な条件下での挙動）**
> 0dBでは、確信度0.9で実際の正答率が86.0%、確信度0.8で67.3%となっており、10dBほどの完璧な一致（対角線）には至らず、やや自信過剰な傾向（カーブが理想線より下）が残ります。
> しかしMCT導入前（確信度0.9で実際30%台等）と比べると劇的に正直になっており、確信度0.7以上の確定タグも **全体の19.5%（10,553件）** 確保できています。「10dBは安定した運用域、0dBは確認フローを多用して凌ぐ最悪条件」という位置づけが明確になりました。
