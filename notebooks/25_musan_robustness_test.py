...
from src.modeling.noise import MusanInjector, add_noise
...
def process_and_eval(X_clean, y_labels, target_classes, title_prefix, snr, pipe, musan, extractor):
    X_features = []
    
    desc = f"Extracting {title_prefix} (SNR={snr if snr is not None else 'Clean'})"
    for x_clean in tqdm(X_clean, desc=desc):
        if snr is not None:
            # Inject MUSAN noise (randomly babble or music)
            x_mixed = musan.inject(x_clean, extractor.sr_orig, snr)
        else:
            x_mixed = x_clean
...
def main():
    ...
    for snr in snr_levels:
        ...
        acc_c, cm_c, total_c, correct_c = process_and_eval(X_c_raw, y_c, cons_classes, "Consonants", snr, pipe, musan, extractor)
        acc_v, cm_v, total_v, correct_v = process_and_eval(X_v_raw, y_v, vowel_classes, "Vowels", snr, pipe, musan, extractor)
        ...
        print(f"Overall Accuracy at {snr_label}: {overall_acc:.2%}")
    ...
    plt.title("Dual Window True Robustness (MUSAN Noise, Matched Training)")
    plt.xlabel("SNR (dB)")
    plt.ylabel("Accuracy")
    ...
    curve_path = os.path.join(os.path.dirname(__file__), 'musan_robustness_curve.png')
